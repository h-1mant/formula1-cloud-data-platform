...
#identify which race_years were affected based on latest batch data, update constructor_standings accordingly
race_results_df = (
    spark.read.format("delta").load(f"{get_adls_path('presentation', 'race_results')}")
    .filter(col("race_year").isin(race_year_list))
)

#update constructor standings for all years in incremental extract 
...
constructor_rank_spec = W.partitionBy("race_year").orderBy(desc("total_points"), desc("wins"))
